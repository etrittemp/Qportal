...
// ============================================
// Extract text from Word with HTML structure
// ============================================
...
function detectQuestionType(questionText, nextLines, strongTexts) {
  ...
  // Check if this question is in strong tags (often indicates special formatting)
  const isStrong = Array.from(strongTexts).some(s => s.includes(questionText.substring(0, 50)));
  ...
  if (lower.match(/scale\s+of\s+\d+\s+to\s+\d+/) ||
    ...
    return 'radio'; // Scale uses radio
  }

  // Ranking questions
  if (lower.match(/rank|order\s+of\s+importance|prioritize/)) {
    return 'textarea'; // Can describe ranking
  }
